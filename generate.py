...
def newsletter(user_id):
    print(f"The letter this time is called by {user_id}")
    print("Newsletter starting up!")
    ...
    # if the user is subscribed or has choosen categories
        ...
        # calling a function to have a topic choosen fro us to create the newsletter on-
        try:
        ...
        # my custom message
        wink, smile= emojize(":winking_face:"), emojize(":smiling_face_with_smiling_eyes:")
        ...
        print("Newsletter returning!") # for error checking
        return {
            "para": format_para,
            "title": title,
        }
...
def topic_chooser(choosen, user_id, db):
    ...
    # calling the category function to get the list of categories selected for this week
    ...
    # sorting out sent and getting the actual sub topics-
    if not sent:
        sent = []
    else:
    ...
    remaining = [t for t in sub_topics if t not in already]
    if not remaining:
        return None
...
# this is the function to get all the content for the newsletter using API
def wiki(topic):
